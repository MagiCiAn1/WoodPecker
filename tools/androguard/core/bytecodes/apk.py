...
class APK(object):
    ...
    def get_icon(self):
        app_elem = self.get_AndroidManifest().getElementsByTagName("application")[0]
        app_icon = app_elem.getAttribute("android:icon")
...
class ARSCParser(object):
    ...
    def get_resource_dimen(self, ate):
        try:
            return [ate.get_value(), "%s%s" % (complexToFloat(ate.key.get_data()), DIMENSION_UNITS[ate.key.get_data() % 6])]
        except Exception, why:
            print ate.key.get_data()
