...
class PathP(object):
  ...
  def get_name(self, cm):
      method = cm.get_method_ref(self.src_idx)
      return method.get_name()

  def get_descriptor(self, cm):
      method = cm.get_method_ref(self.src_idx)
      return method.get_descriptor()
...
class TaintedPackage(object):
    ...
    def get_method(self, name, descriptor):
        ...
        for path in self.paths[ TAINTED_PACKAGE_CALL ]:
            ...
            if dst_name == name and dst_descriptor == descriptor:
                l.append( path )
        return l
...
class TaintedPackages(object):
    ...
    def get_method(self, class_name, name, descriptor):
        try:
            ...
            return []
    #Added by Wood
    def get_method(self, method_name, descriptor):
        try:
            l = []
            for m, i in self.get_packages():
                if i=='Lcom/example/a13251/bugssouce/MainActivity;':
                    print i
                l.extend(m.get_method(method_name, descriptor))
            return l
        except KeyError:
            return []
    #Added by AndroBugs
